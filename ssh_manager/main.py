...
class SSHManagerUI:
    ...
    def draw_list(self, stdscr):
        ...
        if not self.display_items:
            stdscr.addstr(1, 0, "No items found. Press 'a' to add a connection.")
        else:
            for i, (item, item_type, data) in enumerate(self.display_items):
                if i == self.current_index:
                    if item_type == "dir":
                        ...
                        prefix = "> "
                        suffix = ""
                    stdscr.addstr(i + 1, 0, f"{prefix}{item}{suffix}", curses.A_REVERSE)
                else:
                    if item_type == "dir":
                        if item == "..":
                            prefix = "  "
                        ...
                        prefix = "  "
                        suffix = ""
                    stdscr.addstr(i + 1, 0, f"{prefix}{item}{suffix}")
        
        # Draw message if any
        if self.message and self.message_timeout > 0:
            stdscr.addstr(h-1, 0, self.message)
    ...
    def draw_add_form(self, stdscr):
        ...
        # Draw header
        mode = "Edit" if self.mode == "edit" else "Add New"
        stdscr.addstr(0, 0, f"{mode} SSH Connection - Use ↑↓ to navigate, Enter to save, Esc to cancel")
        
        # Draw input fields
        for i, field in enumerate(self.input_fields):
            if i == self.current_field:
                stdscr.addstr(i + 1, 0, f"> {field}: {self.input_values[field]}", curses.A_REVERSE)
            else:
                stdscr.addstr(i + 1, 0, f"  {field}: {self.input_values[field]}")
